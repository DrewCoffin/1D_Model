...
PRO cm3_phaseplot, time, phase, _extra = extra
   for i = 0, n_elements(time) - 2 do $
   if phase[i+1] - phase[i] gt -270  and $
      phase[i+1] - phase[i] lt  270 then $
         oplot, time[i:i+1], phase[i:i+1], _extra = extra
   ;pshift = shift(phase, 1) - phase
   ;ind = where(abs(pshift) lt 270.)
   ;if ind[0] ne -1 then ind = ind[1:n_elements(ind)-1] & $
   ;   oplot, time[ind], phase[ind], _extra = extra
   ;help, time[ind]
end
;-----------------------------------------------------------------------
...
;rj = 7.1492e4 ; [km] is a system variable 

omega = v/(rdist*!rjkm)  ;radians per second at 6.0 Rj
omega_half = v_half/(rdist*!rjkm)
...
PRO cm3_latavg_lag3,n_out,t_out,spfitp_out,s2pfitp_out,s3pfitp_out,opfitp_out,$
            ...
            xwinsize = xwinsize, ywinsize = ywinsize, textbox = textbox, iplot = iplot
...
restore, file = '/Users/shinn/pro/iotorus/avgmixratios.sav'
...
if keyword_set(lon) then l3 = lon ELSE l3 = findgen(nbox)*dtheta+5.
lontemp = l3
...
read_emission_tables,'/Users/shinn/pro/iotorus/tepSII_v6.0.1',temp,den,emisSp
read_emission_tables,'/Users/shinn/pro/iotorus/tepSIII_v6.0.1',temp,den,emisS2p
read_emission_tables,'/Users/shinn/pro/iotorus/tepSIV_v6.0.1',temp,den,emisS3p
read_emission_tables,'/Users/shinn/pro/iotorus/tepOII_v6.0.1',temp,den,emisOp
read_emission_tables,'/Users/shinn/pro/iotorus/tepOIII_v6.0.1',temp,den,emisO2p
...
for j=0,nit-1 do begin 
   ...
   ; print to widget box
   if keyword_set(printiter) and (j MOD 100) eq 0 then begin
      widget_control, textbox, set_value = '% Iteration ' + $
                      string(strtrim(  j, 2), format = '(I5)') + ' of ' + $
                      string(strtrim(nit, 2), format = '(I5)'), /append
   endif
...
; potentially different O/S of o2s_spike
   ...
   ; Time variable hot electron fraction Steffl 2008
   ...
   n.fh = fehot_const * (1. + hote_amp * exp(-(time - hote_t0)^2 / hote_width^2)) * $
          (1. + fehot_amp * cos((l3 - fehot_phase) * !dtor) ) * $
          (1. + s4fehot_amp * cos(((l3 - s4fehot_phase - time * domega) mod 360) * !dtor))
...
;;       ENDIF 
   ...
   if 0 then begin
   if j mod 25 eq 0 then begin
   ; wtorus widget plotting
   if ~setup then begin
      plot, [0, runt1], [0, 360], /nodata, $
            xtit = 'Day of year 2000', $
            /ysty, /xsty, $
            ;yrange = [0, 360], xrange = [0., runt1], $
            ytit = 'System III longitude of peak', $
            ytickint = 60 , yminor = 6, $
            charsize = 1, background = 0
      setup = 1
      ;cm3_phaseplot, tm,  psp, col =  85, thick = 3
      ;cm3_phaseplot, tm, ps2p, col = 160, thick = 3
      ;cm3_phaseplot, tm, ps3p, col = 190, thick = 3
      ;cm3_phaseplot, tm,  pop, col = 254, thick = 3
      ...
      subs = j - indgen(5)
      ;subs = subs[sort(subs)]
      ;cm3_phaseplot, tm[subs],  psp[subs], col =  85, thick = 3
      ;cm3_phaseplot, tm[subs], ps2p[subs], col = 160, thick = 3
      ;cm3_phaseplot, tm[subs], ps3p[subs], col = 190, thick = 3
      ;cm3_phaseplot, tm[subs],  pop[subs], col = 254, thick = 3
      oplot, [tm[subs]],  [psp[subs]], color =  85, thick = 3;, linestyle = 0
      oplot, [tm[subs]], [ps2p[subs]], color = 160, thick = 3;, linestyle = 0
      oplot, [tm[subs]], [ps3p[subs]], color = 190, thick = 3;, linestyle = 0
      oplot, [tm[subs]],  [pop[subs]], color = 254, thick = 3;, linestyle = 0
      ;print, psp[subs]
   endelse
   endif
   endif
   ...
   if ~setup then begin
      ...
      ;subs = j - indgen(5)
      oplot, l3,  ftmix.sp, color =  85, thick = 1
      oplot, l3, ftmix.s2p, color = 160, thick = 1
      oplot, l3, ftmix.s3p, color = 190, thick = 1
      oplot, l3,  ftmix.op, color = 254, thick = 1
      ;stop ; !debug!
   endelse
   endif
...
;      plot, tm, avg(ftmix_out.sp,0), /nodata, yr = [.01,.4], /ysty, $
;            xtit = 'Day of year 2000', xrange = [0., runt1], /xsty, $
...
save,file='cm3_lag'+filename+'.sav',n_out,t_out,h_out,spfitp_out,s2pfitp_out,s3pfitp_out,$
   ...
   if keyword_set(iplot) then begin
      ...
      iplot, view_grid = [1,2], dimensions = [600, 800], name = 'torustwiddle'

      iplot, tm,  psp, color = [  0,  0,240], /overplot, $
             sym_index = 3, sym_size = 2., linestyle = 6, $
             yrange = [0,360], xtitle = 'Day of year 2000', $
             view_title = 'baseline', $
             ytitle = 'System III longitude of peak', ytickint = 60, yminor = 6, $
             view_number = 1;, anisotropic_scale_2d = .2
      iplot, tm, ps2p, color = [  0,240,  0], /overplot, sym_index = 3, sym_size = 2., linestyle = 6
      iplot, tm, ps3p, color = [240,155,  0], /overplot, sym_index = 3, sym_size = 2., linestyle = 6
      iplot, tm,  pop, color = [240,  0,  0], /overplot, sym_index = 3, sym_size = 2., linestyle = 6
      ;plots, [avgtime[ 0], avgtime[ 0]], [!y.crange[0], !y.crange[1]]
      ;plots, [avgtime[41], avgtime[41]], [!y.crange[0], !y.crange[1]]


      ; Plot the amplitude (As a percentage of the contant offset)
      iplot, tm,  ampsp, /overplot, color = [  0,  0,240], thick = 2, $
             yrange = [0,40], xtitle = 'Day of year 2000', $
             ytitle = 'Amplitude of variation (%)', ytickint = 10, $
             view_number = 2;, anisotropic_scale_2d = .2
      iplot, tm, amps2p, /overplot, color = [  0,240,  0], thick = 2
      iplot, tm, amps3p, /overplot, color = [240,155,  0], thick = 2
      iplot, tm,  ampop, /overplot, color = [240,  0,  0], thick = 2
      ;iplot, [avgtime[ 0], avgtime[ 0]], [!y.crange[0], !y.crange[1]]
      ;iplot, [avgtime[41], avgtime[41]], [!y.crange[0], !y.crange[1]]
...
; Plot composition vs time
   ...
   restore,'~/pro/iotorus/fitparvslon_o3.sav'
...
; Phase plots
   ...
   legend,/bottom,/right,['UVIS Data','Model'],psym=[1,0],number=2
   ...
   xyouts,.06,.60,/normal,orient=90,'Longitude of Peak Mixing ratio',charsize=1.5
...
; Amplitude plots
   ...
   legend,pos=[276,.29],['UVIS Data','Model'],psym=[1,0],number=2
   ...
   xyouts,.06,.00,/normal,orient=90,'Amplitude of Azimuthal Variation',charsize=1.5
...
; print runtime in widget text
widget_control, textbox, set_value = 'Runtime of: '+strtrim(systime(1)-runtime,2)+' seconds', /append
