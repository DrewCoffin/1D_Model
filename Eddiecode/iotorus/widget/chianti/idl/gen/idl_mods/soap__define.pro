...
;               Version 2.5, 10-Feb-2012, Hourcle.  Fixed serialization of named arrays of structs 
;
;
...
;   hack to try to deal with IDL8's garbage collection
    if ( !version.release ge 8 ) then x = call_function( 'heap_refcount', /disable )
...
pro soap::cleanup
    ...
    if ( !version.release ge 8 ) then x = call_function( 'heap_refcount', /enable )
...
function soap::post,method,args,debug=debug,_ref_extra=extra

    debug=keyword_set(debug)
    extra_headers=self->header(method)
    add_headers=['User-Agent: IDL/SOAP '+!version.release+' on '+!version.os+'/'+!version.arch,$
                 'Content-Type: text/xml']
    if is_string(extra_headers) then extra_headers=[extra_headers,add_headers] else $
     extra_headers=add_headers
    ...
    purl=self->getprop(/proxy)
    file=(method eq 'GetData') 
    response=sock_post(purl,request,err=err,headers=extra_headers,file=file,_extra=extra)
    if debug then begin
     print, response
...
pro soap__define
    ...
    struct = { soap, INHERITS gen, proxy:'', uri:'', parser:obj_new()}
