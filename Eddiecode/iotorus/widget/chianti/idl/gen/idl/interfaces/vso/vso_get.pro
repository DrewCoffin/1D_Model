...
function vso_get, records, merge=merge, nodownload=nodownload, force=force, quiet=quiet,verbose=verbose,$
                  help=help, methods=methods, rice=rice, norice=norice,nowait=nowait,$
                  filenames=filenames, email=email, site=site,local_file=local_file,_ref_extra=extra
 
    if keyword_set(nowait) then begin
     message,'/NOWAIT deprecated until further notice.',/info
     return,''
    endif
    ...
    vso = obj_new('vso',_extra=extra,protocol=1.0)
...
; seems better compartmentalized like this, for some reason.  -Joe
    ...
    if keyword_set( quiet ) then begin
	; *something* is messing up sock_copy's ability to download from LASP
        network_urls = where ( strmatch( unique_urls, '*lasp.colorado.edu*', /fold ), count,  complement=sock_urls, ncomplement=ncount )
        if ncount then begin
           sock_copy, unique_urls[sock_urls],local_file=local_file,_extra=extra,verbose=1-keyword_set(quiet),protocol=1.0
           downloaded->push,local_file
        endif
        if count then begin
           sock_copy, unique_urls[network_urls],/network,local_file=local_file,_extra=extra,verbose=1-keyword_set(quiet),protocol=1.0
           downloaded->push,local_file
        endif
    endif else begin
...
; yes, I'm walking the array backwards
	    ...
	    network = strmatch( unique_urls[i], '*lasp.colorado.edu*', /fold )
            sock_copy, unique_urls[i], network=network, local_file=local_file, _extra=extra,verbose=1-keyword_set(quiet),protocol=1.0
            downloaded->push,local_file
        endwhile
        print, 'Downloading completed'
    endelse

    if (downloaded->n_elements() gt 0) then $
