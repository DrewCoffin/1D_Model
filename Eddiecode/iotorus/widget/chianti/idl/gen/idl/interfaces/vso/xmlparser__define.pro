...
;               Version 1.3, 27-Dec-2005, Hourcle.  more robust
;               'walktree'                                                                                                      
;               Modified, 11-May-2012, Zarro (ADNET) 
;                - added XMLSTRING string load option to avoid file I/O
; 
...
; Input :                                                                                               
;   XMLSTRING : an xml string or a file name containing the string
; Output :                                                                                              
...
;-- check if XML string entered via a file

file_in=file_test(xmlstring[0])
new_version=since_version('6.4')

;-- if entered as string and old IDL version, then have to use file load

if file_in then file=xmlstring[0]
mk_temp= ~file_in and ~new_version
if mk_temp then begin
 file  = get_temp_file()                                                    
...
if keyword_set(verbose) then message,'parsing XML (new version)...',/cont
if file_in then dom->load, filename=file else dom->load,string=xmlstring[0]
if mk_temp then file_delete,file,/quiet
