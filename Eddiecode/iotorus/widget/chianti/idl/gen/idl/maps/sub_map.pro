...
if use_cursor and ~do_plot then begin
 device,window=ow
...
;-- get pixel dimensions of input map
...
dprint,'% arange: ',arange
 ...
 if ~valid_range(xrange) and ~valid_range(yrange) then begin
  smap=map
  return
 endif
...
if n_elements(dimensions) eq 2 then begin
 ...
 if (x2 ge nx) or (y2 ge ny) then begin
  err='Specified dimensions of sub map exceed input map size.'
  message,err,/cont
  return
 endif
endif
...
for i=0,nmap-1 do begin
 ...
 endelse
 smap=merge_struct(smap,tmap,/no_copy)
endfor
