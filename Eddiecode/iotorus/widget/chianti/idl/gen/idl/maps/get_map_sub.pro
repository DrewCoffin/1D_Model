...
;               XSHIFT,YSHIFT = user-specified translations [def = 0,0] 
;
...
;               Modified 6 June 2012, Zarro (ADNET)
;                - added XSHIFT, YSHIFT keywords.
;
...
function get_map_sub,map,xrange=xrange,yrange=yrange,count=count,err=err,$
        ...
        xp=xp,yp=yp,no_overlap=no_overlap,xcor=xcor,ycor=ycor,$
...
if arg_present(xp) then begin
 xp=get_map_xp(map)
 xp=xp[xstart:xend,ystart:yend]+xshift
endif

if arg_present(yp) then begin
 yp=get_map_yp(map)
 yp=yp[xstart:xend,ystart:yend]+yshift
endif
