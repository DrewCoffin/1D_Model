...
;     SHIFT_AFTER_ROLL = apply shift after roll [def = apply before
;     roll] 
;
; Restrictions:
...
pro plot_map_index,map,contour=cont,overlay=overlay,smooth_width=smooth_width,border=border,$
 ...
 roll=roll,xshift=xshift,yshift=yshift,shift_after_roll=shift_after_roll
...
if shifting or rolling then begin
 saved_map=map
 if keyword_set(shift_after_roll) then begin
