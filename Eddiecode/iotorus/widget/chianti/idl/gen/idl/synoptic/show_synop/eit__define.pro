...
;-- prep level 0 
 ...
 if have_path then read_eit,ofile[i],index,/nodata else $
  self->fits::read,ofile[i],_extra=extra,index=index,/nodata
 
;-- check for multiple images

 nsub=n_elements(index)
 if (nsub gt 1) then begin
  if ~sel_img then image_no=self->select(index)
 ...
 for k=0,nimg-1 do begin
  ...
  if (image_no[k] gt -1) and (image_no[k] lt nsub) then begin
   if have_path then begin
    self->prep,ofile[i],header,data,image_no=image_no[k],_extra=extra
    sz=size(data)
