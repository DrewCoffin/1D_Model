...
if ~remote and is_string(goes_fits_sav) then begin
 ...
 if (count eq 1) then begin
  for i=0,n_elements(search_sats)-1 do begin
   ...
   if verbose then message,'Searching '+sat_name+'...',/info
   message,/reset
  ...
  if verbose then message,err_msg,/info
  return
 endif
endif
...
for i=0,n_elements(search_sats)-1 do begin
 ...
 tsat=search_sats[i]
 sat_name='GOES'+trim(tsat)
 if verbose and network then message,'Searching (remotely) '+sat_name+'...',/info
...
;   at least one.
 ...
 if network then begin
  sock_copy,goes_files,out_dir=goes_dir,local_file=local,_extra=extra,verbose=verbose
...
if verbose and (error eq 1) then begin
 message,err_msg,/info
endif
