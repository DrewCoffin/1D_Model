...
;    25-Jun-2012, Kim. Was losing fraction of sec in data time when inserted as seconds into gdata struct
;      which has long word for time.  Now insert in gdata struct as msec.
;-
;---------------------------------------------------------------------------
...
if network then $
 output='Please wait. Searching ' + server + ' archives across network...' else $
...
; self->output,sdac=sdac
 ...
 for iarch = 0,narch do begin
   ...
   if sdac_used then begin
      ...
      rd_goes,times,trange=[dstart,dend],_extra=extra,err=err,clobber=force,$
...
t1=0.
t2=0.
if ~self->have_gdata() then return
np=n_elements(*self.gdata)
t1=anytim((*self.gdata)[0] / 1000.d0,/tai)
t2=anytim((*self.gdata)[np-1] / 1000.d0,/tai)
