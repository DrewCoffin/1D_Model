      ...
      if verbose then message, 'No input file for week ' + wid + ' found in: ' + indir, /info
            ...
            if (count eq 0) then begin
             ...
             if network then begin
              ...
              sock_copy,sfile,out=temp_dir,verbose=verbose,_extra=extra
             endif
...
if is_dir(comdir) and ~remote then gen=loc_file(gfiles,count=count,/recheck)
if (count eq 0) or remote then begin
 server=goes_server(network=network)
 break_file,gfiles,dsk,dir,name,ext
 if network then begin
  path=ydb_path()+'/'+strlowcase(preweek)+'/'
  if verbose then message,'Checking remote GOES server...',/info
  gen=sock_find(server,name+ext,path=path,protocol=1.0,count=count)
