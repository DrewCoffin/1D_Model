...
if (self.tstart ne self.last_tstart or self.tend ne self.last_tend) then begin
  ...
  *self.data = -1
  if count gt 0 then begin
    self->getfile, file, local_file=local_file, count=count, /use_net, _extra=extra
...
files=self.site->search(self.tstart, self.tend, _extra=extra, count=count, /round, /use_net)
retval = ''
