...
;-- check if RICE-compressed
 ...
 if have_cal and have_path and (level eq 0) then begin
  ...
  self->readfits,rfile,data,_extra=extra,index=index
 endelse
...
if earth_view then self->earth_view,index,map,_extra=extra else $
 if roll_correct then self->roll_correct,index,map,_extra=extra
