...
;                  redirects
;                - added byte-order properties to better handle FITS files
;
...
self->hset,retry=2,buffsize=1024l,protocol='1.1',$
...
pro http::request,url,type=type,err=err,request=request,$
                     _ref_extra=extra,verbose=verbose
...
;if self->use_proxy(server) then begin
; tserver=self->hget(/proxy_host)+':'+trim(self->hget(/proxy_port))
;endif 
if self->hget(/gateway) or self->use_proxy(server) then begin
...
if is_number(protocol) then self.protocol=' HTTP/'+trim(protocol,'(f4.1)')+' '
...
if is_string(proxy,/blank) then begin
 if strtrim(proxy,2) ne ''  then begin
  pst=url_parse(proxy)
  if is_string(pst.host) then self.proxy_host=pst.host 
  if is_number(pst.port) then self.proxy_port=pst.port else self.proxy_port=80
 endif else begin
...
function http::use_proxy,server

;-- check if server is in $no_proxy domain

cserver=self->hget(/server)
if is_blank(cserver) then if is_string(server) then cserver=server
no_proxy=chklog('no_proxy')
if is_string(no_proxy) and is_string(cserver) then $
 if strpos(cserver,no_proxy) gt -1 then return,0b

proxy_host=self->hget(/proxy_host)

return,is_string(proxy_host)
...
if self->is_open(tserver) then goto,skip
...
socket,lun,tserver,tport,/get_lun,error=error,connect_timeout=connect_timeout,$
            read_timeout=read_timeout,swap_endian=swap_endian,swap_if_little_endian=swap_if_little_endian,$
            _extra=extra
...
self->readf,response,err=err,_extra=extra
if is_string(err) then begin
 self->close & return
endif
...
self->head,url,response,err=err,location=location
if is_string(err) then goto,bail

if is_string(location) then durl=location else durl=url

self->request,durl,err=err,_extra=extra
if is_string(err) then goto,bail
...
bail: if is_string(err) then message,err,/info
self->close
...
if code ne 200 then begin
 err='Could not find - '+url & if keyword_set(verbose) then message,err,/info
 self->close
 return,0b
endif
...
if keyword_set(verbose) then message,u[0,chk[0]],/info
code=u[1,chk[0]]
return,fix(code)
end
...
if arg_present(code) then code=self->get_status(response)
...
;-- send HEAD request to check for content

durl=url
self->head,url,response,code=code,size=bsize,location=location,$
                    disposition=disposition,_extra=extra,type=type,err=err
...
if ~clobber and (~check_change or bsize eq 0) and have_file then begin
 if verbose then message,'Local file '+copy_file+' exists (not copied). Use /clobber to recopy.',/info
...
if ~clobber and (osize eq bsize) and (bsize ne 0) then begin
 if verbose then message,'Same size local file '+copy_file+' exists (not copied)',/info
 status=2 & self->close & return
endif
...
if ~clobber and (bsize gt 0) then begin
 if exist(osize) and have_file then begin
  if osize eq bsize then begin 
   if verbose then $
    message,'Same size local file '+copy_file+' exists (not copied, use /clobber)',/info
   status=2 & self->close & return 
  endif
  if verbose then $ 
   message,'Remote & local files differ in size: '+trim(bsize)+','+trim(osize),/info
 endif
endif
...
self->send_request,durl,_extra=extra
response=self->strip_response(_extra=extra)
...
if verbose then begin
 ...
 message,output,/info
 message,'Wrote '+trim(string(counts,'(i10)'))+' bytes to file '+copy_file,/info
endif 
status=1
...
self->close
if status eq 0 then message,'Error copying file.',/info
