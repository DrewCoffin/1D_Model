...
;                 - added /swap_if_little_endian per Landsman suggestion
;
...
if is_string(err) then begin
 message,err,/info & self->close & return
endif
...
message,'GET_SERVER method not included',/info
...
if verbose then begin
 message,'Searching '+anytim2utc(dstart,/vms)+' to '+anytim2utc(dend,/vms),/info
endif
...
if count eq 0 then message,'No files found for specified time(s)',/info else begin
 if keyword_set(verbose) then message,'Found '+trim(count)+' files',/info
endelse
