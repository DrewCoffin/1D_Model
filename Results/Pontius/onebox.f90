...
!    n%fh=n%fh*((mass_loading/ave_loading)**1.0) !Should replace sys4 population.
    v_ion=(((mass_loading*volume*mp)*57.0*(rdist)**5)/(0.1*sqrt(1.0-(1.0/(rdist)))*4.0*PI*1.5*((Rj*1.0e3)**2)*(4.2e-4)**2))
    if( mype .eq. 0) then
    ...
    if (nint(output_it)+1 .eq. i .and. (OUTPUT_MIXR .or. OUTPUT_DENS .or. OUTPUT_TEMP .or. OUTPUT_INTS)) then !Output at set intervals when OUTPUT_MIX is true (from debug.f90)
        ...
        do j=0, LNG_GRID-1
          if( mype .eq. j ) then
            ...
            call OtherOutput(800.0*mass_loading/ave_loading, longitude, day_char, 'LOAD')
          endif
        ...
        if( mype .eq. 0 ) then
          ...
          call OtherOutput(800.0*mass_loading/ave_loading, 360.00, day_char, 'LOAD')
        endif
